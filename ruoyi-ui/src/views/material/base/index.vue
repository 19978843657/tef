<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <right-toolbar :show-search.sync="showSearch" @queryTable="getList" clearable/>
    </el-row>

    <el-table v-loading="loading" :data="list"  height="500px">
...
<!--      <el-table-column label="RFID类型" align="center" prop="rfidType" width="200" :show-overflow-tooltip="true"/>-->
      ...
      <el-table-column label="检验人员" align="center" prop="checkName" width="340px"/>
...
import { list,listw } from '@/api/material/base/index'
...
export default {
  ...
  created() {
    this.getLists()
    this.getUserList()
    console.table(StoreSelect.leth)
  },
  methods: {
    /** 查询物料出库信息列表 */
    getList() {
    ...
    getLists() {
      this.loading = true
      listw(this.queryParams).then(response => {
        this.list = response.rows
        this.total = response.total
        this.loading = false
      })
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('material/base/export', {
        ...this.queryParams
