...
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
<!--          v-hasPermi="['material:repair:add']"-->
<!--          type="primary"-->
<!--          plain-->
<!--          icon="el-icon-plus"-->
<!--          size="mini"-->
...
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:repair:remove']"
          type="danger"
          plain
          icon="el-icon-delete"
          size="mini"
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:repair:import']"
          type="info"
          plain
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:repair:importTemplate']"
          type="warning"
          plain
    ...
    <el-table v-loading="loading" :data="tempList" @selection-change="handleSelectionChange"  height="500px">
      <el-table-column type="selection" width="55" align="center" />
       <el-table-column label="维修ID" align="center" prop="tempRepairMaterialId" />
      ...
      <el-table-column label="系统单号id" align="center" prop="reapirid" width="150"/>
      <el-table-column label="74订单" align="center" prop="qs" width="200"/>
      ...
      <el-table-column label="确认数量" align="center" prop="confirmNumber" width="200">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.confirmNumber" :min="1" :disabled="scope.row.disabled" type="number" size="mini" placeholder="请输入数量" style="width: 100px; margin-right: 5px;" /><el-button v-hasPermi="['material:repair:check']" type="primary" size="mini" @click="handleConfirm(scope.row)">{{ scope.row.disabled ? '检验':'确认' }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="测试报告" align="center" prop="testb" />
      ...
      <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['material:repair:check']"
            size="mini"
            type="text"
            icon="el-icon-edit"
          ...
          >检验</el-button>
          <el-button
            v-hasPermi="['material:repair:edit']"
            size="mini"
            type="text"
            icon="el-icon-edit"
    ...
    <el-dialog :title="title" :visible.sync="open" :close-on-click-modal="false" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="物料" prop="materialNo">
          <el-input v-modelr="form.materialNo"  placeholder="请输入物料编码" />
        </el-form-item>
...
<script>
import { listTemp, getTemp, delTemp, addTemp, updateTemp, check } from '@/api/material/repair/temp'
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/material/repair/importData'
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.tempRepairMaterialId)
    ...
    handleUpdate(row) {
      this.reset()
      const tempRepairMaterialId = row.tempRepairMaterialId || this.ids
      getTemp(tempRepairMaterialId).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.tempRepairMaterialId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const tempRepairMaterialIds = row.tempRepairMaterialId || this.ids
      const materialNos = row.materialNo || this.materialNos
      this.$modal.confirm('是否确认删除维修件入库物料编码为"' + materialNos + '"的数据项？').then(function() {
        return delTemp(tempRepairMaterialIds)
      }).then(() => {
        this.getList()
    ...
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = '维修件入库导入'
      this.upload.open = true
    },
    /** 下载导入模板按钮操作 */
    handleDoload() {
      this.download('/material/repair/importTemplate', {}, `维修件入库导入表.xlsx`)
    },
    // 文件上传中处理
