...
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
<!--          v-hasPermi="['material:zero_in:add']"-->
<!--          type="primary"-->
<!--          plain-->
<!--          icon="el-icon-plus"-->
<!--          size="mini"-->
...
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:zero_in:remove']"
          type="danger"
          plain
          icon="el-icon-delete"
          size="mini"
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:zero_in:import']"
          type="info"
          plain
      ...
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['material:zero_in:importTemplate']"
          type="warning"
          plain
    ...
    <el-table v-loading="loading" :data="tempList" @selection-change="handleSelectionChange"  height="500px">
      <el-table-column type="selection" width="55" align="center" />
       <el-table-column label="零价值ID" align="center" prop="tempZeroMaterialId" />
      ...
      <el-table-column label="确认数量" align="center" prop="confirmNumber" width="200">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.confirmNumber" :min="1" :disabled="scope.row.disabled" type="number" size="mini" placeholder="请输入数量" style="width: 100px; margin-right: 5px;" /><el-button v-hasPermi="['material:zero_in:check']" type="primary" size="mini" @click="handleConfirm(scope.row)">{{ scope.row.disabled ? '检验':'确认' }}</el-button>
        </template>
      </el-table-column>
<!--      <el-table-column label="74单" align="center" prop="qs" width="180" />-->
      <el-table-column label="入库时间" align="center" prop="inputDt" width="180" />
      <el-table-column label="本位币金额" align="center" prop="materialCost" width="180" />
      <el-table-column label="申请工程师" align="center" prop="applicant" />
      ...
      <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['material:zero_in:check']"
            size="mini"
            type="text"
            icon="el-icon-edit"
          ...
          >检验</el-button>
          <el-button
            v-hasPermi="['material:zero_in:edit']"
            size="mini"
            type="text"
            icon="el-icon-edit"
...
<!--        </el-form-item>-->
<!--        <el-form-item label="申请工程师" prop="applicant">-->
<!--          <el-input v-model="form.application" placeholder="请输入申请工程师" />-->
<!--        </el-form-item>-->
...
<script>
import { listTemp, getTemp, delTemp, addTemp, updateTemp, check } from '@/api/material/zeroIn/temp'
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/material/zero_in/importData'
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.tempZeroMaterialId)
    ...
    handleUpdate(row) {
      this.reset()
      const tempZeroMaterialId = row.tempZeroMaterialId || this.ids
      getTemp(tempZeroMaterialId).then(response => {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.tempZeroMaterialId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const tempZeroMaterialIds = row.tempZeroMaterialId || this.ids
      const materialNos = row.materialNo || this.materialNos

      this.$modal.confirm('是否确认删除零价值入库物料编码为"' + materialNos + '"的数据项？').then(function() {
        return delTemp(tempZeroMaterialIds)
      }).then(() => {
        this.getList()
    ...
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = '零价值入库导入'
      this.upload.open = true
    },
    /** 下载导入模板按钮操作 */
    handleDoload() {
      this.download('/material/zero_in/importTemplate', {}, `零价值入库导入表.xlsx`)
    },
    // 文件上传中处理
