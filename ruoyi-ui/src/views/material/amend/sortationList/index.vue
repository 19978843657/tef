      ...
      <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleConfirm(scope.row, 1, '报废')"
          >报废</el-button>
          <el-button
            v-hasPermi="['material:flow:edit']"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleConfirm(scope.row, 2, '维修')"
          >维修</el-button>
          <!-- <el-button
            v-hasPermi="['material:flow:edit']"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleConfirm(scope.row, 3, '紧急维修')"
          >紧急维修</el-button> -->
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { sortationList, sortation } from '@/api/material/amend/sortation'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      sortationList(this.queryParams).then(response => {
    ...
    /** 确认操作 */
    handleConfirm(row, status, flag) {
      const temp = Object.assign({}, row)
      temp.materialStatus = status
      this.$modal.confirm(`是否确认${flag}`).then(function() {
        return sortation(temp)
      }).then(() => {
        this.getList()
